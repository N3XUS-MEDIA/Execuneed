...
**Root Directory must be `apps/web`.** This is the one setting that cannot be
committed to the repo, and getting it wrong is why the first deploy failed.

With Root Directory left at the repository root, Vercel's Next.js builder
looks for `next` in the root `package.json` — where it does not exist, because
this is a workspace and the app lives in `apps/web`. The build fails with
"No Next.js version detected" before it runs anything.

With Root Directory set to `apps/web`, Vercel finds Next.js, detects the pnpm
workspace by walking up to `pnpm-workspace.yaml`, and installs from the
repository root on its own. No `buildCommand`, `installCommand` or
`outputDirectory` override is needed — the earlier `vercel.json` had all three
and they were fighting the framework detection rather than helping it.

`vercel.json` therefore lives at `apps/web/vercel.json`, because Vercel reads
it relative to the Root Directory.

The Prisma client is generated by `packages/db`'s `postinstall`, so it happens
on any install — Vercel, CI, or a fresh clone — rather than depending on a
build command somebody has to remember.
...
`apps/web/vercel.json` declares the digest cron at 05:00 UTC daily — 07:00
