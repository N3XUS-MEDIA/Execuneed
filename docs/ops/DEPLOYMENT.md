...
`vercel.json` stays at the **repository root**. That is where Vercel read it
from on this project even with Root Directory set to `apps/web` — the failing
build proves it, because the build command it ran came from that file. If the
cron ever stops appearing under Settings → Cron Jobs, that assumption is the
